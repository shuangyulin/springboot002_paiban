<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="医护类型" prop="yihuleixing">
          <el-input v-model="ruleForm.yihuleixing" 
              placeholder="医护类型" clearable  :readonly="ro.yihuleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="医护类型" prop="yihuleixing">
              <el-input v-model="ruleForm.yihuleixing" 
                placeholder="医护类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='yihuleixing'){
            this.ruleForm.yihuleixing = obj[o];
	    this.ro.yihuleixing = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yihuleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yihuleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();

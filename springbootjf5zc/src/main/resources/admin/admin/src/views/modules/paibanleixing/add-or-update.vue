<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="班次" prop="banci">
          <el-input v-model="ruleForm.banci" 
              placeholder="班次" clearable  :readonly="ro.banci"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="班次" prop="banci">
              <el-input v-model="ruleForm.banci" 
                placeholder="班次" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `paibanleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.paibanleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
